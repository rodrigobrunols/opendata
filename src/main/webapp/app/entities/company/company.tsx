    ...
    return (
      <div>
        <h2 id="company-heading">
          <Translate contentKey="opendataApp.company.home.title">Companies</Translate>
          <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="opendataApp.company.home.createLabel">Create new Company</Translate>
          </Link>
        </h2>
        <div className="table-responsive">
          {companyList && companyList.length > 0 ? (
              ...
              <tbody>
                {companyList.map((company, i) => (
                  <tr key={`entity-${i}`}>
                    ...
                    <td>{company.name}</td>
                    <td>{company.cnpjNumber}</td>
                    <td>{company.brand ? <Link to={`brand/${company.brand.id}`}>{company.brand.id}</Link> : ''}</td>
            ...
            <div className="alert alert-warning">
              <Translate contentKey="opendataApp.company.home.notFound">No Companies found</Translate>
            </div>
          )}
        </div>
      </div>
